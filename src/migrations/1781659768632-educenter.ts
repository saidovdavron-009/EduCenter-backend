...
export class Educenter1781659768632 implements MigrationInterface {
    ...
    // Production's schema already had all of this migration's tables/types
    // (created before migration tracking started) but its row was never
    // recorded in the `migrations` table, so typeorm treats it as pending.
    // IF NOT EXISTS / duplicate_object guards make re-running it a no-op
    // there while still working as a normal migration on a fresh database.
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "teacher_salaries" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "teacher_id" character varying NOT NULL, "base_amount" numeric(12,2) NOT NULL, "bonus" numeric(12,2) NOT NULL DEFAULT '0', "fine" numeric(12,2) NOT NULL DEFAULT '0', "total_paid" numeric(12,2) NOT NULL DEFAULT '0', "period_start" date NOT NULL, "period_end" date NOT NULL, "is_paid" boolean NOT NULL DEFAULT false, "paid_at" TIMESTAMP WITH TIME ZONE, "note" character varying, CONSTRAINT "PK_9c3bd59d986e1ee7a993506d17e" PRIMARY KEY ("id"))`);
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "public"."teacher_attendance_status_enum" AS ENUM('ON_TIME', 'LATE', 'ABSENT'); EXCEPTION WHEN duplicate_object THEN null; END $$`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "teacher_attendance" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "teacher_id" character varying NOT NULL, "check_in" TIMESTAMP WITH TIME ZONE, "check_out" TIMESTAMP WITH TIME ZONE, "status" "public"."teacher_attendance_status_enum", "date" date NOT NULL, "note" character varying, CONSTRAINT "PK_db5e32ed63fac67b9709ad3be1a" PRIMARY KEY ("id"))`);
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "public"."tasks_status_enum" AS ENUM('TODO', 'IN_PROGRESS', 'DONE'); EXCEPTION WHEN duplicate_object THEN null; END $$`);
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "public"."tasks_priority_enum" AS ENUM('LOW', 'MEDIUM', 'HIGH'); EXCEPTION WHEN duplicate_object THEN null; END $$`);
        ...
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "public"."app_settings_type_enum" AS ENUM('STRING', 'NUMBER', 'BOOLEAN', 'JSON'); EXCEPTION WHEN duplicate_object THEN null; END $$`);
        ...
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "holidays" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "name" character varying NOT NULL, "start_date" date NOT NULL, "end_date" date NOT NULL, "branch_id" character varying, CONSTRAINT "PK_3646bdd4c3817d954d830881dfe" PRIMARY KEY ("id"))`);
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "public"."sms_logs_status_enum" AS ENUM('DELIVERED', 'FAILED', 'PENDING'); EXCEPTION WHEN duplicate_object THEN null; END $$`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "sms_logs" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "phone" character varying NOT NULL, "message" text NOT NULL, "provider_id" character varying, "status" "public"."sms_logs_status_enum", "sent_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_811e3a63f5e14a50475c6e8be3d" PRIMARY KEY ("id"))`);
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "public"."audit_logs_action_enum" AS ENUM('CREATE', 'UPDATE', 'DELETE', 'LOGIN', 'LOGOUT'); EXCEPTION WHEN duplicate_object THEN null; END $$`);
        ...
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "rooms" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "branch_id" character varying NOT NULL, "name" character varying NOT NULL, "capacity" integer NOT NULL, "is_active" boolean NOT NULL DEFAULT true, CONSTRAINT "PK_0368a2d7c215f2d0458a54933f2" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "quizzes" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "group_id" character varying NOT NULL, "title" character varying NOT NULL, "description" text, "time_limit_mins" integer, "is_published" boolean NOT NULL DEFAULT false, "start_at" TIMESTAMP WITH TIME ZONE, "end_at" TIMESTAMP WITH TIME ZONE, "created_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_b24f0f7662cf6b3a0e7dba0a1b4" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "quiz_questions" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "quiz_id" character varying NOT NULL, "question_text" text NOT NULL, "image_url" character varying, "points" numeric(5,2) NOT NULL DEFAULT '1', "sort_order" integer NOT NULL DEFAULT '0', CONSTRAINT "PK_ec0447fd30d9f5c182e7653bfd3" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "quiz_options" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "question_id" character varying NOT NULL, "option_text" text NOT NULL, "is_correct" boolean NOT NULL DEFAULT false, CONSTRAINT "PK_9c59607f100085ab17f0f138926" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "quiz_results" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "quiz_id" character varying NOT NULL, "student_id" character varying NOT NULL, "score" numeric(5,2) NOT NULL DEFAULT '0', "max_score" numeric(5,2), "started_at" TIMESTAMP WITH TIME ZONE, "finished_at" TIMESTAMP WITH TIME ZONE, CONSTRAINT "PK_4ecf38c5bf5b054ccc9f10e438a" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "parents" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "user_id" character varying, "full_name" character varying NOT NULL, "phone" character varying NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "UQ_c94c3cea9b43a18c81269ded41d" UNIQUE ("user_id"), CONSTRAINT "PK_9a4dc67c7b8e6a9cb918938d353" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "student_parents" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "student_id" character varying NOT NULL, "parent_id" character varying NOT NULL, CONSTRAINT "PK_3f3fbf0307e277adf3e90495435" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "lead_sources" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "name" character varying NOT NULL, CONSTRAINT "PK_bc885a4409ec70ee5a810dbbd6f" PRIMARY KEY ("id"))`);
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "public"."leads_status_enum" AS ENUM('NEW', 'CONTACTED', 'TRIAL_LESSON', 'REGISTERED', 'CLOSED'); EXCEPTION WHEN duplicate_object THEN null; END $$`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "leads" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "full_name" character varying NOT NULL, "phone" character varying NOT NULL, "source_id" character varying, "status" "public"."leads_status_enum" NOT NULL DEFAULT 'NEW', "interest_subject_id" character varying, "assigned_admin_id" character varying, "branch_id" character varying, "notes" text, "trial_date" date, "converted_at" TIMESTAMP WITH TIME ZONE, "student_id" character varying, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_cd102ed7a9a4ca7d4d8bfeba406" PRIMARY KEY ("id"))`);
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "public"."call_logs_result_enum" AS ENUM('ANSWERED', 'NO_ANSWER', 'BUSY', 'WRONG_NUMBER'); EXCEPTION WHEN duplicate_object THEN null; END $$`);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "call_logs" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "lead_id" character varying NOT NULL, "admin_id" character varying NOT NULL, "notes" text, "result" "public"."call_logs_result_enum", "next_call_at" TIMESTAMP WITH TIME ZONE, "created_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_aa08476bcc13bfdf394261761e9" PRIMARY KEY ("id"))`);
