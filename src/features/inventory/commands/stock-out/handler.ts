...
import { StockOutRequest } from './request';
import { InventoryLogType, RequestUser } from '../../../../common/types';

@Injectable()
export class StockOutHandler {
  constructor(
  ...
  async execute(dto: StockOutRequest, user: RequestUser) {
    const item = await this.itemRepo.findOne({ where: { id: dto.itemId } });
    if (!item) throw new NotFoundException('Mahsulot topilmadi');
    if (item.stockQuantity < dto.quantity) throw new BadRequestException(`Yetarli miqdor yo'q. Mavjud: ${item.stockQuantity}`);
    ...
    return { id: log.id, itemId: dto.itemId, newStock: item.stockQuantity, quantity: dto.quantity, type: InventoryLogType.OUT };
  }
}